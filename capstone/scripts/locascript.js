...
function addText() {
  ...
  let notes = localStorage.getItem("locationNotes");
  if (notes == null) {
    notesObj = [];
  } else {
  ...
  localStorage.setItem("locationNotes", JSON.stringify(notesObj));
  ...
  notesObj.forEach(function (element, index) {
    html += `
            <div class="noteCard my-2 mx-2 card" style="width: 18rem;">
                    <div class="card-body">
                        <h5 class="card-title">Location ${index + 1}</h5>
...
function get() {
  ...
  let second = JSON.parse(localStorage.getItem("locationNotes"));
